...
**选择vps的另一个原因:**   
可以把自己别的网站, e.g. [unsw.co](https://www.unsw.co) 也放到这个vps上.     
选的香港vps, 肯定还可以做别的事情. 所以在把开销省下来的情况下, 这主机的钱💰还是花的很值得的.    
<br><br>


# 第二步 同步网站内容:
因为是静态网站, 所以只要把生成的静态内容, 放到vps上就行了. Octopress提供了原生的方法: 将`rake deploy`的模式, 从默认的push改为Rsync, 再允许`rake deploy`就会将代码同步到远程的服务器上, 具体的配置如下:   
```yaml
## -- Rsync Deploy config -- 
...
# 第五步 配置证书:
DNS生效需要一会时间, 刚好可以为你的博客加上免费的SSL证书. 我用的是[Let's Encrypt](https://certbot.eff.org/), 真的是无脑一键配置.   
<br><br>
...
<img style="max-height:300px" class="lazy" data-original="/images/blog/170729_hoster/boost2">   
